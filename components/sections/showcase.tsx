...
const showcaseDetails = {
  plan: {
    ...
    details: [
      ...
      "Сложная работа стоит на пике концентрации.",
...
export function ShowcaseSection() {
  ...
  const activeTab = showcaseTabs.find((tab) => tab.key === activeKey) ?? showcaseTabs[0];
  const ActiveIcon = iconMap[activeTab.key as keyof typeof iconMap] ?? Layers;
  const activeDetails =
    showcaseDetails[activeTab.key as keyof typeof showcaseDetails] ?? showcaseDetails.plan;
      ...
      <Container>
        <Reveal className="mb-10 max-w-3xl">
          ...
          <h2 className="mt-5 font-heading text-3xl tracking-tight text-white sm:text-5xl">
            Посмотрите, как Momentum ведет день от первого приоритета до вечернего разбора.
          </h2>
        </Reveal>

        <Card className="relative overflow-hidden rounded-[32px] border-white/15 bg-[#151019]/90 p-4 sm:p-7">
          ...
          <div className="relative lg:hidden">
            <div className="mb-5 rounded-[24px] border border-white/12 bg-white/[0.03] p-4">
              <p className="text-[0.68rem] uppercase tracking-[0.18em] text-white/42">Интерфейс продукта</p>
              <p className="mt-2 font-heading text-2xl leading-tight text-white">
                Один экран для плана, фокуса и разбора.
              </p>
              <p className="mt-3 text-sm leading-relaxed text-white/60">
                Переключайте режимы и сразу видите, как меняется рабочая картина дня.
              </p>
            </div>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    activeKey === tab.key
                      ...
                      : "border-white/12 bg-white/[0.02] text-white/66"
                  )}
                >
                  {tab.label}
                </button>
            ...
            <AnimatePresence mode="wait">
              <motion.div
                key={`${activeTab.key}-mobile-showcase`}
                ...
                className="space-y-4"
              >
                <div className="rounded-[28px] border border-white/12 bg-[#130d13]/82 p-4">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-[0.68rem] uppercase tracking-[0.18em] text-white/42">
                        {activeDetails.windowLabel}
                      </p>
                      <h3 className="mt-2 font-heading text-[1.9rem] leading-[0.98] text-white">
                        {activeTab.headline}
                      </h3>
                    </div>
                    <div className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-accent/32 bg-accent/12 text-accent">
                      <ActiveIcon className="h-5 w-5" />
                    </div>
                  </div>
                  <p className="mt-3 text-sm leading-relaxed text-white/60">{activeTab.description}</p>
                </div>

                <div className="rounded-[28px] border border-white/12 bg-white/[0.03] p-4">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-[0.68rem] uppercase tracking-[0.18em] text-white/42">
                      {activeDetails.status}
                    </p>
                    <span className="rounded-full border border-white/12 bg-black/18 px-3 py-1 text-[0.68rem] text-white/60">
                      Momentum Workspace
                    </span>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {activeTab.metrics.map((metric, index) => (
                    ...
                    >
                      <p className="text-[0.6rem] uppercase tracking-[0.16em] text-white/38">
                        {metric.label}
                      </p>
                      <p className="mt-2 font-heading text-[1.45rem] leading-none text-white">
                        {metric.value}
                      </p>
                    </motion.div>
                  ))}
                ...
                <div className="rounded-[28px] border border-accent/22 bg-accent/8 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-2 font-heading text-3xl text-white">{activeDetails.signalValue}</p>
                    </div>
                    <span className="rounded-full border border-accent/24 bg-black/20 px-3 py-1 text-[0.64rem] text-accent">
                      обновлено сейчас
                    </span>
                  </div>
                  <div className="mt-4 h-2 rounded-full bg-white/10">
                    <motion.div
                      key={`${activeTab.key}-mobile-signal`}
                  ...
                  <p className="mt-4 text-sm leading-relaxed text-white/72">{activeDetails.note}</p>
                  <div className="mt-5 flex h-20 items-end gap-2">
                    {activeDetails.pressure.map((bar, index) => (
                      <div key={`${activeTab.key}-mobile-bar-${chartLabels[index]}`} className="flex flex-1 items-end">
                        <motion.div
          ...
          <div className="relative hidden lg:block">
            <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-[0.18em] text-white/44">Интерфейс продукта</p>
            ...
            <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <h3 className="font-heading text-3xl leading-tight text-white">{activeTab.headline}</h3>
                  ...
                  <div className="mt-6 rounded-2xl border border-accent/22 bg-accent/8 p-4">
                    ...
                    <p className="mt-2 text-sm leading-relaxed text-white/72">{activeDetails.note}</p>
                  </div>
                </motion.div>
              </AnimatePresence>
                ...
                >
                  <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/12 bg-white/[0.03] px-4 py-3">
                    <div>
                      ...
                      <p className="mt-1 font-heading text-xl text-white">{activeDetails.status}</p>
                    </div>
                  ...
                  <div className="grid gap-3 lg:grid-cols-[1.02fr_0.98fr]">
                    <div className="rounded-[24px] border border-white/12 bg-black/18 p-4">
                      <p className="text-xs uppercase tracking-[0.16em] text-white/44">Ритм дня</p>
                      <div className="mt-4 space-y-3">
                        {activeTab.timeline.map((item, index) => (
                          <motion.div
                            key={`${activeTab.key}-${item.time}-${item.title}`}
                          ...
                          >
                            <div className="flex items-start justify-between gap-3">
                              ...
                              <div className="text-right">
                                <p className="text-xs uppercase tracking-[0.13em] text-white/46">{item.time}</p>
                                <span
                                  className={cn(
                    ...
                    <div className="grid gap-3">
                      <div className="rounded-[24px] border border-white/12 bg-white/[0.03] p-4">
                        ...
                        <div className="mt-3 flex items-center justify-between gap-3">
                          <p className="font-heading text-4xl text-white">{activeDetails.signalValue}</p>
                      ...
                      <div className="rounded-[24px] border border-white/12 bg-white/[0.03] p-4">
                        <p className="text-xs uppercase tracking-[0.16em] text-white/44">Нагрузка по часам</p>
                        <div className="mt-5 flex h-28 items-end gap-2">
                          {activeDetails.pressure.map((bar, index) => (
                            <div key={`${activeTab.key}-bar-${chartLabels[index]}`} className="flex flex-1 items-end">
                              <motion.div
                                className="w-full rounded-t-2xl bg-gradient-to-t from-accent to-white/78"
                                initial={{ height: 0 }}
                                animate={{ height: `${bar}%` }}
                      ...
                      <div className="rounded-[24px] border border-white/12 bg-black/18 p-4">
                        <p className="text-xs uppercase tracking-[0.16em] text-white/44">Сигнал системы</p>
                        <p className="mt-2 text-sm leading-relaxed text-white/72">{activeDetails.note}</p>
                      </div>
                    </div>
                  </div>
