...
function PlanCard({ index, billing }: PlanCardProps) {
  const plan = pricingPlans[index];
  const displayPrice = billing === "monthly" ? plan.monthly : plan.yearly;
  const note = billing === "monthly" ? "оплата помесячно" : plan.yearlyNote;
    ...
    >
      ...
      <div className="relative flex h-full flex-col">
        ...
        <div className="mt-6 flex items-end gap-2">
          <span className="font-heading text-5xl leading-none text-white">{displayPrice}</span>
          <span className="mb-1 text-sm text-white/60">/месяц</span>
        </div>
        <p className="mt-2 text-sm text-white/52">{note}</p>
        {billing === "yearly" && plan.savings ? <p className="mt-2 text-sm text-accent">{plan.savings}</p> : null}
      ...
      <Container>
        <div className="mb-10 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <Reveal className="max-w-2xl">
            ...
            <p className="mt-4 max-w-xl text-sm leading-relaxed text-white/66">
              Можно стартовать бесплатно, перейти на полный рабочий ритм или подключить команду к единой системе планирования.
            </p>
          </Reveal>

          <div className="w-full rounded-full border border-white/12 bg-white/[0.03] p-1 sm:w-auto">
            <div className="grid grid-cols-[1fr_1fr_auto] items-center gap-1">
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isActive ? "bg-accent text-black" : "text-white/62 hover:text-white"
                    )}
                  >
                    {option.label}
                  </button>
              ...
              <span className="pr-2 text-right text-[0.68rem] uppercase tracking-[0.16em] text-white/40">
                выгоднее
              </span>
            </div>
          </div>
        ...
        <div className="lg:hidden">
          <div className="mb-4 flex items-center justify-between gap-3 px-1">
            <p className="text-sm text-white/58">Выберите формат оплаты и пролистайте планы.</p>
            <span className="text-[0.68rem] uppercase tracking-[0.18em] text-white/34">свайп</span>
          </div>
          <div className="-mx-4 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-2 no-scrollbar">
            {pricingPlans.map((plan, index) => (
              ...
              >
                <PlanCard index={index} billing={billing} />
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <Card className="mt-6 overflow-hidden rounded-[30px] border-white/12 bg-[#17121b]/86 p-4 sm:p-5">
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.18em] text-white/44">Сравнение планов</p>
              <h3 className="mt-2 font-heading text-2xl text-white">Что входит в каждый тариф</h3>
            </div>
          ...
          <div className="space-y-3 lg:hidden">
            {pricingComparison.map((row) => (
              <div key={row.feature} className="rounded-[24px] border border-white/10 bg-black/18 p-4">
                  ...
                  {row.values.map((value, index) => (
                    <div
                      ...
                      className={cn(
                        ...
                        pricingPlans[index]?.highlight
                          ? "border-accent/22 bg-accent/8"
                    ...
                    >
                      <p
                        className={cn(
                          ...
                          pricingPlans[index]?.highlight ? "text-accent" : "text-white/38"
                        )}
                      >
                        {pricingPlans[index]?.name}
                      </p>
                      <p className="mt-2 text-sm leading-relaxed text-white/72">{value}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <table className="min-w-full border-separate border-spacing-y-2">
              <thead>
                <tr className="text-left">
                  <th className="px-3 py-2 text-xs uppercase tracking-[0.16em] text-white/40">Возможность</th>
